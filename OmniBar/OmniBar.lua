...
function OmniBar:Delete(button, keepProfile)
	local key = type(button) == "table" and button.arg or button
	local bar = _G[key]
...
function OmniBar_UpdateIcons(self)
	for i = 1, self.numIcons do
		-- Set show text
		self.icons[i].cooldown:SetHideCountdownNumbers(self.settings.cooldownCount and false or true)
